...
import wrl.java.util.ObjectComparator;
...
public class TreeMap extends AbstractMap implements SortedMap, Cloneable //, java.io.Serializable
 {
	 ...
	 * keys inserted into the map must be <i>mutually comparable</i> by the
	 ...
	 * new map must implement the <tt>Comparable</tt> interface. Furthermore,
	 * all such keys must be <i>mutually comparable</i>:
...
//		System.out.println("21 "+((p == null) ? "null" : p.value));
...
//		System.out.println("34");
	...
	public Set keySet() {
		if (keySet == null) {
			keySet = new AbstractSet() {
						public wrl.java.util.Iterator iterator() {
	 ...
	 * the returned set is a <tt>Map.Entry</tt>.  The set is backed by this map,
	 ...
	 * <tt>fromKey</tt> and <tt>toKey</tt> are equal, the returned sorted map is
	 * empty.)  The returned sorted map is backed by this map, so changes in the
	 * returned sorted map are reflected in this map, and vice-versa. The
	 ...
	 * 
	 * <p>
	 * Note: this method always returns a <i>half-open range</i> (which includes
	 * its low endpoint but not its high endpoint).  If you need a <i>closed
	 * range</i> (which includes both endpoints), and the key type allows for
	 * calculation of the successor a given key, merely request the subrange
	 * from <tt>lowEndpoint</tt> to <tt>successor(highEndpoint)</tt>. For
	 * example, suppose that <tt>m</tt> is a sorted map whose keys are strings.
	 ...
	 * endpoint.  If you need a view that does contain this endpoint, and the
	 * key type allows for calculation of the successor a given key, merely
	 * request a headMap bounded by <tt>successor(highEndpoint)</tt>. For
	 * example, suppose that suppose that <tt>m</tt> is a sorted map whose keys
	 ...
	 * request a tailMap bounded by <tt>successor(lowEndpoint)</tt>. For For
	 * example, suppose that suppose that <tt>m</tt> is a sorted map whose keys
	 * are strings.  The following idiom obtains a view containing all of the
...
//		System.out.println("40 "+((comparator == null) ? "null" : comparator.getClass().toString()));
		if (comparator == null) {
			if (k1 instanceof String) comparator = new wrl.java.util.StringComparator();
			else if (k1 instanceof Integer) comparator = new wrl.java.util.IntegerComparator();
		}
		if (comparator == null) {
			System.out.println("comparator for "+k1.getClass().toString()+" is not implemented");
			System.out.println("please implement your own one and call map constructor with its instance");
			throw new wrl.java.util.ComparatorException();
		}
		else return comparator.compare(k1, k2);
//		return ((comparator == null) ? ((Comparable) k1).compareTo(k2) : comparator.compare(k1, k2));
// Comparable sa naloaduje z Sun Javy, v ms jave nie je a to sekne celu ms javu. Andy
	}
	...
	/*    private void writeObject(java.io.ObjectOutputStream s)
	  ...
	  for (wrl.java.util.Iterator i = entrySet().iterator(); i.hasNext(); ) {
	...
	*/
	...
	void addAllForTreeSet(SortedSet set, Object defaultVal) {
		//      try {
		buildFromSorted(set.size(), set.iterator(), /*null,*/
			defaultVal);
	 ...
	 * null). 2) An iterator of keys.         (it != null, defaultVal != null).
	 * 3) A stream of alternating serialized keys and values. (it == null,
	 * defaultVal == null). 4) A stream of serialized keys. (it == null,
	 * defaultVal != null). It is assumed that the comparator of the TreeMap is
	 * already set prior to calling this method.
	 *
	 ...
	 * 				from this iterator.
	 * @param defaultVal if non-null, this default value is used for each value
	 * 				in the map.  If null, each value is read from iterator or stream,
	 * 				as described above.
	 */
	private void buildFromSorted(int size, wrl.java.util.Iterator it, 
	 ...
	 * @return DOCUMENT ME!
	 */
	private static Entry buildFromSorted(int level, int lo, int hi, int redLevel, wrl.java.util.Iterator it, 
	/*java.io.ObjectInputStream str,*/
	Object defaultVal)
	/*throws  java.io.IOException, ClassNotFoundException*/  {
		/*
		...
		if (lo < mid) {
			left = buildFromSorted(level + 1, lo, mid - 1, redLevel, it, /*str,*/
					defaultVal);
		}
		...
		if (mid < hi) {
			Entry right = buildFromSorted(level + 1, mid + 1, hi, redLevel, it, /*str,*/
				defaultVal);
	...
	private class SubMap extends AbstractMap implements SortedMap /*, java.io.Serializable */ {
		 ...
		 * Creates a new SubMap object.
		 *
		 * @param fromStart DOCUMENT ME!
		 * @param fromKey DOCUMENT ME!
		 * @param toEnd DOCUMENT ME!
		 * @param toKey DOCUMENT ME!
		 */
		...
		public Object get(Object key) {
			...
			Object obj = TreeMap.this.get(key);
		...
		private class EntrySetView extends AbstractSet {
			...
			public int size() {
				if ((size == -1) || (sizeModCount != TreeMap.this.modCount)) {
					...
					wrl.java.util.Iterator i = iterator();
	 ...
	 * TreeMap Iterator.
	 */
	private class Iterator implements wrl.java.util.Iterator {
		private int type;
		 ...
		 * @param firstExcluded DOCUMENT ME!
		 */
